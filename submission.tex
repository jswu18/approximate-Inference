...
%\centering
%\includegraphics[scale=0.5]{outputs/q2/f-extrapolation-untrained}
%\caption{Extrapolation of $CO_2$ concentration levels}
%\label{fig:f-extrapolation-untrained}
%\end{figure}
%
%\begin{figure}[h]
...
\lstinputlisting[language=Python]{src/models/mean_field_learning.py}
...
For $\textbf{A}$ we choose a factorised conjugate prior:

\[P( \textbf{A}| \eta) = \prod_{k=1}^{K}P( \textbf{A}_{: k}| \eta)\]

where $\textbf{A}_{: k} \in \mathbb{R}^{D \times 1}$ is the $k^{th}$ column of $\textbf{A}$. For each column we choose:

\[P( \textbf{A}_{: k}| \eta) = \mathcal{N}(\textbf{A}_{: k}| \mu_{\textbf{A}_{: k}}, \Sigma_{\textbf{A}_{: k}}) =\frac{1}{(2\pi)^{\frac{D}{2}} |\Sigma_{\textbf{A}_{: k}}|^{\frac{1}{2}}} \exp\left(-\frac{1}{2}(\textbf{A}_{: k} - \mu_{\textbf{A}_{: k}})^T \Sigma_{\textbf{A}_{: k}}^{-1}(\textbf{A}_d - \mu_{\textbf{A}_d})\right)\]

a Gaussian prior with diagonal covariance $\Sigma_{\textbf{A}_{: k}} = \alpha_k^{2}\textbf{I}$ and mean zero, so we can simplify:

\[P( \textbf{A}_{: k}| \alpha_k) = (2\pi\alpha_k^{2})^{\frac{-D}{2}} \exp\left(-\frac{\textbf{A}_{: k}^T \textbf{A}_{: k}}{2\alpha_k^{2}} \right)\]

For $\Psi}$ we choose a conjugate prior:

\[P( \Psi| \eta) = \prod_{d=1}^D InvGamma(\Psi_{dd}| a, b) = \prod_{d=1}^D \frac{b^{a}}{\Gamma(a)}\Psi_{dd}^{-a-1}\exp(-\frac{b}{\Psi_{dd}})\]

a product of inverse gamma distributions on $\Psi$ where we assume $\Psi$ is a diagonal matrix.
...
\tab \tab \tab \tab \tab + \sum_{i=1}^K -\frac{D}{2} \log (2 \pi\alpha_k^2)  -\frac{\textbf{A}_{: k}^T \textbf{A}_{: k}}{2\alpha_k^{2}}\\
\tab \tab \tab \tab \tab + \sum_{d=1}^D a \log b + (-a - 1) \log\Psi_{dd} - \frac{b}{\Psi_{dd}} - \log \Gamma(a)\\
\end{array}
\]
...
%\tab \tab \tab \tab \tab + \sum_{d=1}^D a_d \log b_d + (-a_d - \frac{3}{2}) \log\Psi_{dd} - \frac{b_d}{\Psi_{dd}} - \log \Gamma(a_d)\\
%\end{array}
%\]
...
\[
q_{\Psi}(\Psi) \propto \left( \prod_{d=1}^D \frac{b^{a}}{\Gamma(a)}\Psi_{dd}^{-a-1}\exp(-\frac{b}{\Psi_{dd}}) \right)\exp \left\langle  - \frac{1}{2} \sum_{d=1}^{D}\log\Psi_{dd}   -\frac{1}{2}(\textbf{x}-\textbf{As})^T \Psi^{-1} (\textbf{x}-\textbf{As}) \right\rangle_{q_{\textbf{s}}(\textbf{s}) q_{\neg \Psi(\theta)}}
\]

We see:

\[
q_{\Psi}(\Psi) \propto  \prod_{d=1}^D \frac{b^{a}}{\Gamma(a)}\Psi_{dd}^{-(a+\frac{1}{2})-1}\exp\left(-\frac{b+\frac{1}{2} \left\langle (x_d-\textbf{A}_{d :}\textbf{s})^2 \right\rangle_{q_{\textbf{s}}(\textbf{s})q_{\textbf{A}_{d :}}(\textbf{A}_{d :})}}{\Psi_{dd}}\right) }
\]
...
\[
q_{\Psi}(\Psi) = \prod_{d=1}^D  InvGamma\left(\Psi_{dd} \left| a+\frac{1}{2}, b+\frac{1}{2} \left\langle (x_d-\textbf{A}_{d :}\textbf{s})^2 \right\rangle_{q_{\textbf{s}}(\textbf{s})q_{\textbf{A}_{d :}}(\textbf{A}_{d :})}\right)
\]

For $q_{\textbf{A}_{: k}}(\textbf{A}_{: k})$:

\[q_{\textbf{A}_{: k}}(\textbf{A}_{: k})  \propto P(\textbf{A}_{: k})\exp \left\langle \log P(\textbf{x}, \textbf{s}, \pi, \textbf{A}, \Psi | \eta) \right\rangle_{q_{\textbf{s}}(\textbf{s}) q_{\neg \textbf{A}_{: k}}(\theta)}\]

Substituting the appropriate terms:

\[q_{\textbf{A}_{: k}}(\textbf{A}_{: k}) \propto \exp\left(-\frac{\textbf{A}_{: k}^T \textbf{A}_{: k}}{2\alpha_k^{2}} \right) \exp \left\langle -\frac{1}{2}(\textbf{x}-\textbf{A}_{: k} s_k)^T \Psi^{-1} (\textbf{x}-\textbf{A}_{: k} s_k) \right\rangle_{q_{\textbf{s}}(\textbf{s}) q_{\neg \textbf{A}_{: k}}(\theta)}\]

\[q_{\textbf{A}_{: k}}(\textbf{A}_{: k}) \propto \exp\left(-\frac{\textbf{A}_{: k}^T \textbf{A}_{: k}}{2\alpha_k^{2}} \right) \exp \left\langle -\frac{1}{2} \textbf{A}_{: k}^T\frac{1 }{ s_k^2 \Psi_{dd}}\textbf{A}_{: k}  - 2 \frac{s_k}{\Psi_{dd}} \textbf{x}^T\textbf{A}_{: k} \right\rangle_{q_{\textbf{s}}(\textbf{s}) q_{\neg \textbf{A}_{: k}}(\theta)}\]

We see that

\[q_{\textbf{A}_{: k}}(\textbf{A}_{: k}) = \mathcal{N}\left(\mu_{\textbf{A}_{: k}}, \Sigma_{\textbf{A}_{: k}}\right)}\]

where:

\[ \Sigma_{\textbf{A}_{: k}} = \left(\frac{1}{\alpha_k^2} + \frac{1}{s_k^2\Psi_{dd}}\right)^{-1} \textbf{I}\]

and

\[\mu_{\textbf{A}_{: k}} = \Sigma_{\textbf{A}_{: k}}^{-1} \frac{s_k}{\Psi_{dd}}\textbf{x}\]

By optimising with respect to the the distributions $\Psi$ and $\alpha$ in turn causes some $\alpha_i^2$ to diverge, the number of remaining $\alpha_i^2$ provide our determination for the value of $K$. This is automatic relevance determination through factor analysis.
...
%\[\mathcal{M}_{j \rightarrow i} = \tilde{f}_j(s_j) \prod_{k\in ne(j), k\neq i}^{K}}\tilde{g}_{kj, \neg s_k}(s_j)  \sum_{\substack{s_m \in \{0, 1\} \\ m \in \{1, ..., j-1, j+1,... K\}}} \prod_{k\in ne(j), k\neq i}^{K}} \tilde{g}_{kj, \neg s_j}(s_k)\]

%\[\mathcal{M}_{j \rightarrow i}(s_j) = \tilde{f}_j(s_j) \prod_{k\in ne(j), k\neq i}^{K}}\tilde{g}_{kj, \neg s_k}(s_j) \prod_{k\in ne(j), k\neq i}^{K}} \left(\sum_{s_k \in \{0, 1\} } \tilde{g}_{kj, \neg s_j}(s_k) \right)\]
%
%\[\mathcal{M}_{j \rightarrow i}(s_j) = \tilde{f}_j(s_j) \prod_{k\in ne(j), k\neq i}^{K}}\tilde{g}_{kj, \neg s_k}(s_j)\]
...
%
%\[
%\log g_{ij}(s_i, s_j) q_{\neg \tilde{g}_{ij}(s_i, s_j)}(\textbf{s}) \propto W_{ij} s_i s_j+ A_{s_i} s_i + B_{s_j} s_j
%\]
%
%where:
%
%\[A_{s_i} = (K-2)\log \left(\frac{\theta_i}{1-\theta_i} \right) + \log \left(\frac{\lambda_i}{1-\lambda_i} \right)\]
%
%and
%
%\[B_{s_j} = (K-2)\log \left(\frac{\theta_j}{1-\theta_j} \right) + \log \left(\frac{\lambda_j}{1-\lambda_j} \right)\]
...
\caption{Latent factors learned with EP/Loopy-BP}
